...
<body>
  ...
  <div id="auth-screen">
    <div class="auth-box">
      ...
      <div class="form-row">
        <input type="email" id="auth-email" class="f-input-dark" placeholder="Email">
      </div>
      <div class="form-row">
  ...
  <div id="app-screen" style="display:none;">
    ...
    <div class="main">
      <div id="page-dashboard" class="page active">
        ...
        <!-- FITUR 1: CARD PENGELUARAN TERBESAR -->
      ...
      <div id="page-riwayat" class="page">
        ...
        <div class="card">
          ...
          <!-- FITUR 3: FILTER RENTANG TANGGAL KUSTOM -->
          <div class="filter-bar" style="flex-wrap: wrap;">
            <select id="flt-type" class="f-input-dark" style="width: auto;" onchange="renderAll()">
              <option value="">Semua Tipe</option>
              <option value="income">Pemasukan Saja</option>
              <option value="expense">Pengeluaran Saja</option>
              <option value="debt">Hutang Saja</option>
              <option value="recv">Piutang Saja</option>
              <option value="transfer">Transfer Saja</option>
            </select>
            <input type="date" id="flt-start" class="f-input-dark" style="width: auto;" onchange="renderAll()" title="Dari Tanggal">
            <span style="color:var(--text3); font-weight:800;">-</span>
            <input type="date" id="flt-end" class="f-input-dark" style="width: auto;" onchange="renderAll()" title="Sampai Tanggal">
            <input type="text" id="flt-search" class="f-input-dark" placeholder="Cari keterangan / kategori..." oninput="renderAll()" style="flex: 1; min-width: 200px;">
  ...
  <script type="module">
    ...
    window.resetAccount = function() {
      Swal.fire({
        title: 'Ganti Akun?', 
        text: "PIN di perangkat ini akan direset dan lu harus login Email lagi.",
        icon: 'warning', 
    ...
    window.addTx = async function() { 
      ...
      if (!amt || (!cat && curType !== 'transfer') || isNaN(amt)) {
        ...
        if (!cat && curType !== 'transfer') { 
          catInput.classList.add('shake-error'); 
          setTimeout(() => catInput.classList.remove('shake-error'), 400); 
        }
        return Swal.fire({
          position: 'center', 
          icon: 'warning', 
          title: 'Oops...', 
          text: 'Isi nominal dan kategori!', 
          showConfirmButton: false, 
          timer: 2000, 
          background: 'var(--card)', 
          color: 'var(--text)', 
    ...
    window.editTx = function(id) { 
      ...
      setTimeout(() => { 
        if (document.getElementById('f-cat')) document.getElementById('f-cat').value = t.category; 
      }, 50); 
      ...
      window.scrollTo(0, currY); // Mencegah lompat ke atas otomatis
    };

    /* FITUR 4: TOMBOL DUPLIKAT */
    window.duplicateTx = function(id) {
      if (navigator.vibrate) navigator.vibrate(20);
      Swal.fire({
        title: 'Duplikat Transaksi?',
        text: "Data akan disalin ke form untuk dibuat transaksi baru.",
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
          const t = txs.find(x => x.id === id); 
          if (!t) return; 
          editId = null; // reset ID agar jadi transaksi baru
          selType(t.type); 
          document.getElementById('f-amount').value = t.amount; 
          ...
          document.getElementById('f-note').value = t.note === '-' ? '' : t.note; 
          window.setRealLocalTime(); 
          document.getElementById('save-btn').textContent = 'SIMPAN DUPLIKAT'; 
          document.getElementById('cancel-edit-btn').style.display = 'block'; 
    ...
    /* FUNGSI FITUR 1: RENDER PENGELUARAN TERBESAR */
    function renderTopSpending() {
      const now = new Date();
      const currentMonthStr = now.toISOString().slice(0, 7);
      const expensesThisMonth = txs.filter(t => t.type === 'expense' && t.date.startsWith(currentMonthStr));
      ...
      if (container) {
        if (topAmt > 0) {
          container.innerHTML = `
            <div class="card" style="padding:16px; margin-bottom:24px; display:flex; align-items:center; gap:16px; background: rgba(248, 113, 113, 0.05); border-color: rgba(248, 113, 113, 0.3);">
              <div style="font-size:28px;">💸</div>
              <div>
                <div style="font-size:10px; font-weight:800; color:var(--text3); text-transform:uppercase; letter-spacing:0.5px; margin-bottom: 2px;">PENGELUARAN TERBESAR BULAN INI</div>
                <div style="font-size:15px; font-weight:700; color:var(--red2);">${topCat} <span style="color:var(--text); font-family:'JetBrains Mono', monospace;">- ${fmt(topAmt)}</span></div>
              </div>
            </div>`;
        } else {
          container.innerHTML = '';
        ...
        <div class="recent-item" data-id="${t.id}">
          ...
          <div class="ri-right-wrap">
            ...
            <div class="action-btns">
              ${actionBtn}
              <button class="edit-btn-recent" onclick="duplicateTx('${t.id}')">DUPLIKAT</button>
    ...
    function renderList(container, arr) { 
      container.innerHTML = arr.length 
        ? arr.map(t => createTxCard(t)).join('') 
        : '<div style="padding:40px;text-align:center;color:#888;font-size:12px;">Kosong</div>'; 
    }
    ...
    function renderMetrics() {
      ...
      document.getElementById('metric-cards').innerHTML = `
        ...
        <div class="m-card cnt">
          ...
          <div class="m-val">${ts.count} transaksi</div>
          <div class="m-sub" style="font-weight:700;">${ts.inc > 0 ? fmt(ts.inc) : 'Kosong'}</div>
          <div class="m-bar"><div class="m-bar-fill" style="width:${ts.count > 0 ? 100 : 0}%"></div></div>
        </div>
      `;
    }
      ...
      html += `
        <div style="grid-column: 1 / -1; display: grid; grid-template-columns: 1fr 1fr; gap: inherit;">
          <div class="w-card" style="border-color:rgba(251, 191, 36, 0.5); background:rgba(251, 191, 36, 0.05);">
            ...
            <div class="w-val min">${fmt(hutangBal)}</div>
            <div style="font-size: 8px; color: var(--text3); font-family: 'JetBrains Mono', monospace; margin-top: 2px;">${getUSD(hutangBal)}</div>
          </div>
          <div class="w-card" style="border-color:rgba(59, 130, 246, 0.5); background:rgba(59, 130, 246, 0.05);">
            ...
            <div class="w-val min">${fmt(piutangBal)}</div>
            <div style="font-size: 8px; color: var(--text3); font-family: 'JetBrains Mono', monospace; margin-top: 2px;">${getUSD(piutangBal)}</div>
          </div>
        </div>`;
    ...
    function mkChart(id, labels, incData, expData) { 
      ...
      charts[id] = new Chart(c, {
        ...
        data: {
          labels,
          datasets: [
            { label: 'Pemasukan', data: incData, backgroundColor: isLight ? '#10B981' : '#10B981', borderRadius: 4 },
            { label: 'Pengeluaran', data: expData, backgroundColor: isLight ? '#F87171' : '#F87171', borderRadius: 4 }
          ]
        },
        options: {
          responsive: true,
          ...
          scales: {
            ...
            y: {
              ticks: { color: isLight ? '#888' : '#888', font: { size: 10 }, callback: v => Intl.NumberFormat('id-ID', { notation: 'compact' }).format(v) },
    ...
    function showWeek(k) { 
      ...
      const inc = new Array(7).fill(0);
      const exp = new Array(7).fill(0); 
    ...
    function showMonth(k) { 
      ...
      const labels = [];
      const inc = new Array(dim).fill(0);
      const exp = new Array(dim).fill(0); 
    ...
    function renderYearly() { 
      ...
      document.getElementById('year-sel').innerHTML = keys.map((k, i) => 
        `<button class="p-btn${i === 0 ? ' active' : ''}" onclick="selYear('${k}',this)">${k}</button>`
      ).join(''); 
    ...
    /* FUNGSI RENDER SEMUA & FILTER RENTANG TANGGAL */
    window.renderAll = function() { 
      ...
      // Filter Tanggal
      if (start) arr = arr.filter(t => t.date >= start);
      if (end) {
        const endDate = new Date(end);
        endDate.setDate(endDate.getDate() + 1);
        const endStr = endDate.toISOString().slice(0, 10);
        arr = arr.filter(t => t.date < endStr);
      }
  ...
  <script>
    window.addEventListener('DOMContentLoaded', (event) => {
      ...
      // MENCEGAH JUMP/SCROLL OTOMATIS SAAT EDIT
